...
import com.hhp.lectureapp.lecture.persistence.LectureSessionRepositoryImpl;
...
//    }
    ...
    public void applyLectureNotFoundLectureIdTest() {
        ...
        given(lectureSessionRepository.findByIdAndLectureId(any(Long.class), any(Long.class))).willReturn(Optional.empty());

        CustomException exception = assertThrows(CustomException.class, () -> {
            lectureService.applyLecture(1, 1, 1);
        });

        assertEquals(ErrorCode.NOT_FOUND_LECTURE.getMsg(), exception.getMsg());
